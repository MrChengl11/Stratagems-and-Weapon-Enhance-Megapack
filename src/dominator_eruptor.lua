-- HD2-Addon: mods/dsh/dominator_eruptor
...
local state = rawget(_G, "DominatorEruptorRound")
if state then return end

local REVISION = "der-v1"
...
rawset(_G, "DominatorEruptorRound", state)
...
--   %LOCALAPPDATA%/Hd2ProjRecon/dominator_eruptor.cfg
...
local function read_overrides()
    ...
    local ok, f = pcall(io.open, base .. "/Hd2ProjRecon/dominator_eruptor.cfg", "r")
...
if not ok_ffi or not ffi then
    state.status = "no_ffi"
    print("[DominatorEruptor] FFI unavailable")
    return
end
...
local function log(line)
    print("[DominatorEruptor] " .. line)
    if not out_dir then return end
    local mode = log_started and "a" or "w"
    log_started = true
    local ok, f = pcall(io.open, out_dir .. "/DominatorEruptor.log", mode)
    if ok and f then
...
local function write_status()
    ...
    write_file("STATUS.txt", table.concat({
        ...
        "log=" .. tostring(out_dir) .. "/DominatorEruptor.log",
